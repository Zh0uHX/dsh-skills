...
__export(index_exports, {
  ...
  inject: () => inject,
  name: () => name,
  publishSkillObservation: () => publishSkillObservation
});
module.exports = __toCommonJS(index_exports);
var import_node_path2 = require("node:path");
var import_dsh_home_paths = require("@deepseek-ai/dsh-home-paths");
...
// src/catalog.ts
var ORIGIN = "https://www.skills.sh";
var ALLOWED_HOSTS = /* @__PURE__ */ new Set(["skills.sh", "www.skills.sh"]);
var FALLBACK_DESCRIPTION = "\u6682\u65E0\u7B80\u4ECB";
var MAX_FILES = 2e3;
var MAX_BUNDLE_BYTES = 20 * 1024 * 1024;
...
function canonicalSkill(value) {
  ...
  if (value.source !== source || value.skillId !== skillId || typeof value.name !== "string" || !text(value.name, 200) || !Number.isSafeInteger(value.installs) || value.installs < 0) {
  ...
  return {
    ...
    ...typeof value.description === "string" ? { description: text(value.description, 2e3) } : {}
  };
}
...
function structuredDescription(value, depth = 0) {
  ...
  if ((type === "SoftwareApplication" || Array.isArray(type) && type.includes("SoftwareApplication")) && typeof value.description === "string") return text(value.description, 2e3) || void 0;
...
function descriptionFromPage(html) {
  ...
  for (const match of html.matchAll(/<meta\b[^>]*>/gi)) {
    ...
    if ((attrs.name === "description" || attrs.property === "og:description") && attrs.content) {
      return text(attrs.content, 2e3) || FALLBACK_DESCRIPTION;
    }
  }
...
function safeFilePath(path2) {
  return path2.length <= 512 && !/[\\\u0000-\u001f\u007f:]/.test(path2) && path2.split("/").every((part) => part !== "" && part !== "." && part !== "..");
}
function validateDshMetadata(metadata, skillId) {
  if (!record(metadata) || typeof metadata.name !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(metadata.name) || metadata.name !== skillId || typeof metadata.description !== "string" || !metadata.description.trim()) throw snapshotError();
...
function validateSnapshot(payload, skill) {
  if (!record(payload) || typeof payload.hash !== "string" || !/^[a-f0-9]{64}$/i.test(payload.hash) || !Array.isArray(payload.files) || payload.files.length === 0 || payload.files.length > MAX_FILES) throw snapshotError();
  const files = [];
  const seen = /* @__PURE__ */ new Set();
  let bytes = 0;
  for (const file of payload.files) {
    if (!record(file) || typeof file.path !== "string" || !safeFilePath(file.path) || typeof file.contents !== "string" || seen.has(file.path.toLowerCase())) throw snapshotError();
  ...
  if (!frontmatter || frontmatter.length > 1e5) throw snapshotError();
  try {
    const document = (0, import_yaml.parseDocument)(frontmatter, { uniqueKeys: true });
...
var SkillsShSource = class {
  ...
  async search(query) {
    ...
    const results = new Array(skills.length);
    let next = 0;
    await Promise.all(Array.from({ length: Math.min(4, skills.length) }, async () => {
      while (next < skills.length) {
        ...
        try {
          results[index] = await this.details(skill);
        } catch {
          results[index] = { ...skill, description: FALLBACK_DESCRIPTION };
        }
      }
    }));
    return results;
  ...
  async json(path2, limit) {
    const raw = await this.request(new URL(path2, ORIGIN), limit);
    try {
      return JSON.parse(raw);
    } catch {
      throw new SkillError("SOURCE_UNAVAILABLE", "Skills.sh \u8FD4\u56DE\u4E86\u65E0\u6548\u6570\u636E\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002", 502);
    }
  }
  async request(initial, limit) {
    let url = initial;
...
var PRIVATE_DIR = ".dsh-skills";
var OWNER_FILE = ".dsh-skills-managed.json";
var MAX_FILES2 = 256;
...
async function statIfExists(file) {
  try {
    return await (0, import_promises.lstat)(file);
  } catch (error) {
...
function validatePath(value) {
  if (typeof value !== "string" || !value || Buffer.byteLength(value) > 1024 || /[\\:\u0000-\u001f\u007f]/u.test(value) || import_node_path.default.posix.isAbsolute(value)) {
    unsafe("\u6280\u80FD\u5305\u542B\u4E0D\u5B89\u5168\u7684\u6587\u4EF6\u8DEF\u5F84\u3002");
  }
  const segments = value.split("/");
  if (segments.length > 16 || segments.some((segment) => !segment || segment === "." || segment === ".." || /[. ]$/u.test(segment) || Buffer.byteLength(segment) > 255 || /^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/iu.test(segment))) {
    unsafe(`\u4E0D\u652F\u6301\u7684\u6280\u80FD\u6587\u4EF6\u8DEF\u5F84\uFF1A${value}`);
  }
  if (value.normalize("NFC").toLowerCase() === OWNER_FILE) unsafe("\u6280\u80FD\u4F7F\u7528\u4E86\u63D2\u4EF6\u4FDD\u7559\u7684\u6587\u4EF6\u540D\u3002");
}
function validateFiles(value) {
  if (!Array.isArray(value) || !value.length || value.length > MAX_FILES2) {
    throw new SkillError("INVALID_BUNDLE", `\u6280\u80FD\u5FC5\u987B\u5305\u542B 1\u2013${MAX_FILES2} \u4E2A\u6587\u4EF6\u3002`);
  }
...
function hashFiles(files) {
  return (0, import_node_crypto.createHash)("sha256").update(JSON.stringify(files)).digest("hex");
}
function frontmatterName(contents) {
  const yaml = contents.match(/^\uFEFF?---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*(?:\r?\n|$)/)?.[1];
  if (yaml === void 0) return void 0;
  try {
    if (Buffer.byteLength(yaml) > 1e5) throw new Error("frontmatter too large");
    const document = (0, import_yaml2.parseDocument)(yaml, { schema: "core", uniqueKeys: true });
    if (document.errors.length) throw new Error("invalid YAML");
    const value = document.toJS({ maxAliasCount: 0 });
    if (!value || typeof value !== "object" || !("name" in value)) return void 0;
    return typeof value.name === "string" && /^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(value.name) ? value.name : void 0;
  } catch {
...
function catalog(value) {
  ...
  for (const key of ["id", "skillId", "name", "source", "url"]) {
    if (typeof input[key] !== "string" || !input[key] || Buffer.byteLength(input[key]) > 2048 || /[\u0000-\u001f\u007f]/u.test(input[key])) throw new SkillError("INVALID_SKILL", `\u6280\u80FD\u5B57\u6BB5\u65E0\u6548\uFF1A${key}`);
  }
  if (typeof input.installs !== "number" || !Number.isFinite(input.installs) || input.installs < 0 || input.description !== void 0 && (typeof input.description !== "string" || input.description.length > 16e3)) {
...
var SkillStore = class {
  ...
  async list() {
    return this.locked(async () => {
      ...
      for (const record2 of records) {
        if (!await this.assertPath(record2.directory, true)) {
          record2.problem = "\u6280\u80FD\u76EE\u5F55\u5DF2\u4E0D\u5B58\u5728\uFF1B\u786E\u8BA4\u5378\u8F7D\u53EF\u4EC5\u6E05\u7406\u5B89\u88C5\u8BB0\u5F55\uFF0C\u7136\u540E\u91CD\u65B0\u5B89\u88C5\u3002";
          continue;
        }
        await this.assertOwned(record2);
      }
      return records;
    });
  }
  async install(input, confirmed = false) {
    const skill = catalog(input);
    return this.locked(async () => {
      const records = await this.readRecords();
      const previous = records.find((record2) => record2.id === skill.id);
  ...
  async checkUpdates() {
    return this.locked(async () => {
      ...
      for (const record2 of records) {
        try {
          await this.assertOwned(record2);
          const bundle = await this.download(record2.skill);
          const modified = hashFiles(await this.readSkillFiles(record2.directory)) !== record2.contentHash;
          const changed = hashFiles(bundle.files) !== record2.contentHash;
          results.push({
            id: record2.id,
          ...
          results.push({ id: record2.id, status: "error", message: error instanceof Error ? error.message : "\u68C0\u67E5\u66F4\u65B0\u5931\u8D25\u3002" });
        }
      }
      return results;
    });
  }
  async update(id, confirmed = false) {
    return this.locked(async () => {
      const records = await this.readRecords();
  ...
  async uninstall(id, confirmed = false) {
    return this.locked(async () => {
      const records = await this.readRecords();
      const record2 = this.find(records, id);
      ...
      await this.assertOwned(record2);
      await this.inspectTree(record2.directory);
      const backup = this.privatePath(`removed-${(0, import_node_crypto.randomUUID)()}`);
      await this.move(record2.directory, backup);
      try {
        await this.saveRecords(records.filter((item) => item.id !== id));
      } catch (error) {
        await this.move(backup, record2.directory);
        throw error;
      }
  ...
  directory(id) {
    return import_node_path.default.join(this.root, `skill-${(0, import_node_crypto.createHash)("sha256").update(id).digest("hex").slice(0, 32)}`);
  }
  privatePath(name2) {
    return import_node_path.default.join(this.root, PRIVATE_DIR, name2);
  }
  confirm(message) {
    throw new SkillError("CONFIRMATION_REQUIRED", message, 409);
  }
  find(records, id) {
    const record2 = records.find((item) => item.id === id);
    if (!record2) throw new SkillError("NOT_INSTALLED", "\u6B64\u6280\u80FD\u6CA1\u6709\u672C\u63D2\u4EF6\u7684\u5B89\u88C5\u8BB0\u5F55\u3002", 404);
    return record2;
  }
  /** Reject symlinks at every existing ancestor, including the configured root. */
  async ensureRoot() {
    const parsed = import_node_path.default.parse(this.root);
    let current = parsed.root;
    for (const component of this.root.slice(parsed.root.length).split(import_node_path.default.sep).filter(Boolean)) {
      current = import_node_path.default.join(current, component);
      let stats2 = await statIfExists(current);
      if (!stats2) {
        if (current !== this.root) throw new SkillError("INVALID_ROOT", "\u6280\u80FD\u6839\u76EE\u5F55\u7684\u7236\u76EE\u5F55\u5FC5\u987B\u5DF2\u5B58\u5728\u3002");
        const parent = await (0, import_promises.lstat)(import_node_path.default.dirname(current));
        ...
        stats2 = await (0, import_promises.lstat)(current);
      }
      if (!stats2.isDirectory() || stats2.isSymbolicLink()) unsafe("\u6280\u80FD\u76EE\u5F55\u53CA\u5176\u7236\u8DEF\u5F84\u4E0D\u80FD\u662F\u7B26\u53F7\u94FE\u63A5\u3002");
    }
    const stats = await (0, import_promises.lstat)(this.root);
    if (await (0, import_promises.realpath)(this.root) !== this.root || this.anchor && (stats.ino !== this.anchor.ino || stats.dev !== this.anchor.dev)) {
  ...
  async assertPath(file, allowMissing = false) {
    await this.ensureRoot();
    const relative = import_node_path.default.relative(this.root, file);
    ...
    for (let i = 0; i < segments.length; i++) {
      current = import_node_path.default.join(current, segments[i]);
  ...
  async locked(operation) {
    await this.ensureRoot();
    await this.makeDirectory(import_node_path.default.join(this.root, PRIVATE_DIR));
    ...
    while (true) {
      ...
      try {
        await (0, import_promises.mkdir)(lock, { mode: 448 });
        identity2 = await (0, import_promises.lstat)(lock);
        break;
      } catch (error) {
        if (!hasCode(error, "EEXIST")) throw error;
    ...
    try {
      ...
      if (current?.ino !== identity2.ino || current.dev !== identity2.dev) unsafe("\u63D2\u4EF6\u9501\u5728\u64CD\u4F5C\u671F\u95F4\u53D1\u751F\u53D8\u5316\u3002");
      await (0, import_promises.rmdir)(lock);
    }
  }
  async readText(file, limit) {
  ...
  async readRecords() {
    ...
    try {
      const state = JSON.parse(await this.readText(file, MAX_STATE_BYTES));
      if (!state || typeof state !== "object" || !("version" in state) || state.version !== 1 || !("records" in state) || !Array.isArray(state.records) || state.records.length > MAX_RECORDS) throw new Error();
      const ids = /* @__PURE__ */ new Set();
      return state.records.map((entry) => {
        if (!entry || typeof entry !== "object") throw new Error();
        const record2 = entry;
        const skill = catalog(record2.skill);
        if (typeof record2.id !== "string" || record2.id !== skill.id || ids.has(record2.id) || record2.directory !== this.directory(record2.id) || !/^[a-f0-9]{64}$/u.test(record2.contentHash) || typeof record2.upstreamHash !== "string" || record2.upstreamHash.length > 256 || typeof record2.installedAt !== "string" || !Number.isFinite(Date.parse(record2.installedAt)) || typeof record2.updatedAt !== "string" || !Number.isFinite(Date.parse(record2.updatedAt))) throw new Error();
        ids.add(record2.id);
        return {
          id: record2.id,
          skill,
          directory: record2.directory,
          contentHash: record2.contentHash,
          upstreamHash: record2.upstreamHash,
  ...
  async writeNew(file, contents) {
    if (await this.assertPath(file, true)) unsafe("\u5199\u5165\u76EE\u6807\u5DF2\u5B58\u5728\u3002");
    const handle = await (0, import_promises.open)(file, import_node_fs.constants.O_WRONLY | import_node_fs.constants.O_CREAT | import_node_fs.constants.O_EXCL | import_node_fs.constants.O_NOFOLLOW, 384);
    try {
  ...
  async saveRecords(records) {
    ...
    const temporary = this.privatePath(`state-${(0, import_node_crypto.randomUUID)()}.tmp`);
    try {
      ...
      if (await this.assertPath(temporary, true)) await (0, import_promises.unlink)(temporary);
    }
  }
  async assertVacant(directory) {
    if (await this.assertPath(directory, true)) {
      throw new SkillError("UNMANAGED_DIRECTORY", "\u76EE\u6807\u76EE\u5F55\u5DF2\u5B58\u5728\u4E14\u4E0D\u5C5E\u4E8E\u672C\u63D2\u4EF6\uFF0C\u4E0D\u80FD\u8986\u76D6\u3002", 409);
    }
  }
  async assertOwned(record2) {
    const stats = await this.assertPath(record2.directory);
    if (!stats?.isDirectory()) unsafe("\u5DF2\u5B89\u88C5\u6280\u80FD\u8DEF\u5F84\u4E0D\u662F\u76EE\u5F55\u3002");
    const marker = import_node_path.default.join(record2.directory, OWNER_FILE);
    if (!await this.assertPath(marker, true)) {
      throw new SkillError("UNMANAGED_DIRECTORY", "\u6280\u80FD\u6240\u6709\u6743\u6807\u8BB0\u7F3A\u5931\uFF0C\u4E0D\u80FD\u4FEE\u6539\u8BE5\u76EE\u5F55\u3002", 409);
    }
    let owner;
    try {
      owner = JSON.parse(await this.readText(marker, 8192));
    } catch (error) {
      if (error instanceof SkillError) throw error;
    }
    if (!owner || typeof owner !== "object" || !("version" in owner) || owner.version !== 1 || !("id" in owner) || owner.id !== record2.id) {
  ...
  async download(skill) {
    ...
    if (returned.id !== skill.id || returned.source !== skill.source || returned.skillId !== skill.skillId || typeof bundle.upstreamHash !== "string" || bundle.upstreamHash.length > 256) {
  ...
  async assertNameAvailable(files, ownDirectory) {
    const name2 = frontmatterName(files.find((file) => file.path === "SKILL.md").contents);
    if (name2 === void 0) return;
    await this.ensureRoot();
    for (const entry of await (0, import_promises.readdir)(this.root, { withFileTypes: true })) {
      const candidate = import_node_path.default.join(this.root, entry.name);
      if (entry.name === PRIVATE_DIR || candidate === ownDirectory) continue;
      const stats = await this.assertPath(candidate, true);
      if (!stats) continue;
      const entrypoint = stats.isDirectory() ? import_node_path.default.join(candidate, "SKILL.md") : stats.isFile() && entry.name.endsWith(".md") ? candidate : void 0;
      if (!entrypoint || !await this.assertPath(entrypoint, true)) continue;
      let existingName;
      try {
        existingName = frontmatterName(await this.readText(entrypoint, MAX_FILE_BYTES));
      } catch (error) {
      ...
      if (existingName === name2) {
        throw new SkillError("SKILL_NAME_CONFLICT", `\u6280\u80FD\u540D\u79F0\u300C${name2}\u300D\u5DF2\u88AB ${entry.name} \u4F7F\u7528\u3002DSH \u6309\u540D\u79F0\u53BB\u91CD\uFF0C\u8BF7\u5148\u5904\u7406\u540C\u540D\u6280\u80FD\u3002`, 409);
      }
    }
  }
  async inspectTree(directory) {
    const files = [];
    let nodes = 0;
    const visit = async (current, depth) => {
      if (depth > 20 || files.length > 2e3) unsafe("\u672C\u5730\u6280\u80FD\u76EE\u5F55\u8FC7\u5927\u6216\u8FC7\u6DF1\uFF0C\u65E0\u6CD5\u5B89\u5168\u5904\u7406\u3002");
      const stats = await this.assertPath(current);
      if (!stats?.isDirectory()) unsafe("\u6280\u80FD\u76EE\u5F55\u7C7B\u578B\u65E0\u6548\u3002");
      for (const entry of await (0, import_promises.readdir)(current, { withFileTypes: true })) {
        if (++nodes > 4e3) unsafe("\u672C\u5730\u6280\u80FD\u76EE\u5F55\u5305\u542B\u8FC7\u591A\u6587\u4EF6\u6216\u76EE\u5F55\u3002");
        const child = import_node_path.default.join(current, entry.name);
  ...
  async readSkillFiles(directory) {
    const files = [];
    for (const file of await this.inspectTree(directory)) {
      const relative = import_node_path.default.relative(directory, file).split(import_node_path.default.sep).join("/");
    ...
    const remove = async (current) => {
      for (const entry of await (0, import_promises.readdir)(current, { withFileTypes: true })) {
        ...
        if (stats?.isDirectory()) await remove(child);
          ...
          await this.assertPath(child);
          await (0, import_promises.unlink)(child);
        } else unsafe("\u6E05\u7406\u76EE\u5F55\u5305\u542B\u4E0D\u5B89\u5168\u7684\u6587\u4EF6\u3002");
      }
      await this.assertPath(current);
      await (0, import_promises.rmdir)(current);
    };
    await remove(directory);
  }
  async replace(records, skill, previous) {
    const bundle = await this.download(skill);
    const stage = this.privatePath(`stage-${(0, import_node_crypto.randomUUID)()}`);
    const backup = this.privatePath(`backup-${(0, import_node_crypto.randomUUID)()}`);
    ...
    const record2 = {
      ...
      skill: bundle.skill,
      directory,
      contentHash: hashFiles(bundle.files),
      upstreamHash: bundle.upstreamHash,
      installedAt: previous?.installedAt ?? now,
      updatedAt: now
    };
    const nextRecords = [...records.filter((item) => item.id !== skill.id), record2];
    ...
    try {
      ...
      for (const file of bundle.files) {
        ...
        for (const segment of file.path.split("/").slice(0, -1)) {
          parent = import_node_path.default.join(parent, segment);
          await this.makeDirectory(parent);
        }
        await this.writeNew(import_node_path.default.join(stage, ...file.path.split("/")), file.contents);
      }
      await this.writeNew(import_node_path.default.join(stage, OWNER_FILE), JSON.stringify({ version: 1, id: skill.id }) + "\n");
      await this.assertNameAvailable(bundle.files, directory);
      if (previous) {
        await this.assertOwned(previous);
...
// src/index.ts
var name = "dsh-skills";
var inject = ["webServer", "connection", "fs"];
var Config = import_schemastery.default.object({ dshHome: import_schemastery.default.string() });
var ROUTE = "/api/dsh-skills";
...
async function readBody(req) {
  ...
  try {
    ...
    throw new SkillError("INVALID_JSON", "\u8BF7\u6C42\u4E0D\u662F\u6709\u6548\u7684 JSON\u3002");
  }
...
function skillOf(value) {
  ...
  return {
    ...
    ...typeof item.description === "string" ? { description: item.description.slice(0, 8e3) } : {}
  };
}
function idOf(body) {
...
async function publishSkillObservation(ctx, directory) {
  const target = await ctx.fs.resolve((0, import_node_path2.join)(directory, "SKILL.md"));
...
function apply(ctx, config = {}) {
  const root = (0, import_node_path2.join)((0, import_dsh_home_paths.resolveDshHome)(config.dshHome), "skills");
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        switch (action) {
          case "/details":
            reply(res, 200, { skill: await source.details(skillOf(body.skill)) });
            break;
          case "/install": {
          ...
          case "/check-updates":
            reply(res, 200, { updates: await store.checkUpdates() });
            break;
          case "/update": {
